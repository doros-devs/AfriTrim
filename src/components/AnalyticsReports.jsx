...
const AnalyticsReports = () => {
  ...
  const [analyticsDataFromDB, setAnalyticsDataFromDB] = useState({});
  ...
  const fetchAnalytics = async () => {
    try {
      const response = await fetch(`http://localhost:5555/api/barbershop/1/analytics`, {
        method: 'GET',
        headers: {
  ...
  return (
    <div className="w-full bg-black text-white flex flex-col p-4 overflow-hidden">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6 w-full mb-6 flex-grow">
        {[
          {
            title: "Total Revenue",
            value: `$${analyticsDataFromDB.totalRevenue}`,
          },
          {
            title: "Total Payments",
            value: `$${analyticsDataFromDB.totalPayments}`,
          },
          {
            title: "Total Appointments",
            value: analyticsDataFromDB.totalAppointments,
          },
          {
            title: "New Clients This Week",
            value: analyticsDataFromDB.newClientsThisWeek,
          },
          {
            title: "Top Performing Barber",
            value: analyticsDataFromDB.topPerformingBarber,
          },
        ].map((item, index) => (
          <div
            key={index}
